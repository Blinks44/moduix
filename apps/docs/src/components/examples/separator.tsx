...
import { Separator, type SeparatorSize, type SeparatorVariant } from 'moduix';
...
export const separatorOverrideCssProperties: CssPropertyInput[] = [
  ['--separator-border-style', 'solid', 'Controls the separator border style.'],
...
export function SeparatorAsChildExample() {
  return (
    <div className={styles.section}>
      <span className={styles.text}>Before native rule</span>
      <Separator asChild>
        <hr className={styles.nativeRule} />
      </Separator>
      <span className={styles.text}>After native rule</span>
    </div>
  );
}
