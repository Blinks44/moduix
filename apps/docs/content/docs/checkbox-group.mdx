...
Keep `CheckboxGroupItemControl` and `CheckboxGroupItemLabel` inside the same
`CheckboxGroupItem` so focus and labeling stay aligned.
...
| `CheckboxGroupLabel`       | Optional group heading.                                                             |
| `CheckboxGroupList`        | Optional list wrapper for checkbox rows.                                            |
| `CheckboxGroupItem`        | Optional row wrapper for one control + one text label.                              |
...
`CheckboxGroupItemControl` composes `Checkbox`, so it also supports checkbox props
(`size`, `nativeButton`, `render`, `name`, `value`, `checkedIcon`,
`indeterminateIcon`, `classNames`, `slotProps`).
