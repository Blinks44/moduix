...
`ContextMenu` combines a trigger surface with popup content. Keep interactive rows inside
`ContextMenuContent` so focus and close behavior stay centralized.
...
| Part                           | Role                                                                         |
| ------------------------------ | ---------------------------------------------------------------------------- |
| `ContextMenu`                  | Root state machine for open state, focus management, and close interactions. |
| `ContextMenuTrigger`           | Opens the menu on right click or long press.                                 |
| `ContextMenuContent`           | Popup container with positioning props and internal service layers.          |
...
| `ContextMenuLinkItem`          | Link row with the same menu highlight/focus behavior.                        |
| `ContextMenuSeparator`         | Divider between related item groups.                                         |
| `ContextMenuSubmenu*`          | Nested actions (`Trigger`, `TriggerIcon`, `Content`).                        |
| `ContextMenuGroup*` / controls | Group labels plus radio/checkbox controls with indicator slots.              |

`ContextMenuContent` includes internal service slots (`portal`, optional `backdrop`, `positioner`, `popup`).
Most use cases should style visible parts only; use `classNames`, `slotProps`, and `withBackdrop` for slot-level overrides.

## Composition

Pass positioning props (`sideOffset`, `align`, `collisionPadding`, `positionMethod`) directly to `ContextMenuContent`.
Use `className` for the visible popup and `classNames` for internal service slots. `container`, `slotProps`,
and `withBackdrop` expose matching Base UI escape hatches:
...
Legacy aliases `portalProps`, `backdropProps`, and `positionerProps` remain available for backward compatibility.
