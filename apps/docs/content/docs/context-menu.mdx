...
`ContextMenuContent` renders `Portal`, optional `Backdrop`, `Positioner`, and `Popup`
internally, so the public composition stays focused on menu content. Pass positioning props such as
`sideOffset`, `align`, `collisionPadding`, and `positionMethod` directly to `ContextMenuContent`.
