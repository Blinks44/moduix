...
| `ContextMenuContent`           | Default popup wrapper. Renders `Portal`, `Positioner`, and `Popup`, with the built-in arrow available through `showArrow`. |
| `ContextMenuArrow`             | Built-in visual pointer for the popup. Enable it with `showArrow` or replace it in custom composition.                     |
...
props `side`, `sideOffset`, `align`, `alignOffset`, `arrowPadding`, `collisionAvoidance`,
...
Pass positioning props to `ContextMenuContent` when the default structure is enough. Add
`showArrow` when the popup should visually point back to the trigger surface.
