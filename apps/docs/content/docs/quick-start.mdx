...
Create `components.json` in the project root with the following content. This is a temporary
workaround until `shadcn` fixes custom registry base init for non-Tailwind setups:
...
This gives the `shadcn` CLI the aliases and project configuration it needs to place generated files
in the correct `components` and `lib` locations.
