...
Before you add registry items, initialize `shadcn` in the project:

```bash
npx shadcn@latest init
```

This creates `components.json` and sets up the project configuration that the CLI uses to place
generated files in the correct `components` and `lib` locations.
...
`@/lib`. If your project uses different aliases, import the generated files from the matching paths.
