...
Use `Drawer` for edge-attached panels in moduix. It covers two common patterns:

- **Drawer behavior:** edge placement, swipe-to-dismiss, optional edge swipe area, and snap points.
- **Sheet behavior:** structured content (header/body/footer), side and bottom layouts, and
  non-modal or persistent setups.
...
`DrawerContent` is the composed popup container and renders `portal`, `backdrop`, `viewport`,
`handle`, and the inner `content` slot.
...
In most cases, keep default layering/gestures and style visible parts with `className`
(`DrawerContent`, `DrawerHeader`, `DrawerBody`, `DrawerFooter`). Use `classNames`
(`portal`, `backdrop`, `viewport`, `handle`, `content`) only for internal slot customization.

## Composition

Use `Drawer` for root state/behavior props (`open`, `defaultOpen`, `onOpenChange`, `modal`,
`swipeDirection`, `snapPoints`, `snapPoint`, `onSnapPointChange`). Use `DrawerContent`
for the composed surface; its `className` and popup props are passed to the visible panel.

Use `withBackdrop`, `withHandle`, `variant`, `snapLayout`, and `container` for common behavior.
`className` styles the visible popup, `classNames` styles internal slots, and `slotProps`
is needed only for non-class slot props:
...
| `withHandle`              | `true`  | `true`, `false`                     |
| `variant`                 | `bleed` | `bleed`, `island`                   |
| `snapLayout`              | `false` | `true`, `false`                     |
| `disableInitialAnimation` | `false` | `true`, `false`                     |
| `container`               | `null`  | `HTMLElement`, `ShadowRoot`, `null` |

For root props inherited from Base UI (`modal`, `openOnSwipe`, `dismissible`, `snapPoints`, and
others), `Drawer` keeps Base UI defaults unless you override them.
