...
import {
  ...
  CustomHandleAndIconDrawerExample,
  DrawerExample,
...
`DrawerContent` renders the portal, backdrop, viewport, popup, handle, and inner content region for
you. Use `withBackdrop`, `withHandle`, `variant`, `snapLayout`, and `container` for common behavior.
Use `classNames` to style internal service slots such as `backdrop`, `viewport`, `handle`, and
`content`. Use `portalProps`, `backdropProps`, `viewportProps`, `contentProps`, and `handleProps`
...
### Custom Handle And Icon

Use `classNames.handle` for the built-in handle. Prefer the handle CSS variables for color, opacity,
size, and radius so overrides stay independent from CSS layer order. Pass children to
`DrawerSnapToggle` to replace the default icon.
...
<Preview>
  ...
  <Preview.Code>
    {`
      ...
      export function CustomHandleDrawerDemo() {
        ...
        return (
          <Drawer snapPoints={[...snapPoints]} snapPoint={snapPoint} onSnapPointChange={setSnapPoint}>
            ...
            <DrawerContent
              snapLayout
              classNames={{ handle: styles.customHandle }}
            >
              <DrawerHeader>
                ...
                <DrawerDescription>
                  The handle uses classNames.handle, and the snap toggle accepts custom icon children.
                </DrawerDescription>
              </DrawerHeader>
            </DrawerContent>
          </Drawer>
