...
`CloseButton` is a single icon button slot. It renders a default close icon, or custom icon
...
`CloseButton` is a thin styled wrapper around Base UI `Button`. Use `children` to replace the
