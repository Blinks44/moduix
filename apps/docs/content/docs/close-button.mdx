...
`CloseButton` is built on Base UI Button. Use:

- `children` to replace the default icon.
- `render` and `nativeButton` for custom element composition.
- `focusableWhenDisabled` when a disabled control must stay in tab order.
...
Use `focusableWhenDisabled` if disabled state should remain keyboard-focusable.
