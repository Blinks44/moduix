...
`showArrow={false}` to hide the arrow, `arrow` to replace its graphic, and placement props such as
`side` or `sideOffset` for positioning. `portalProps`, `positionerProps`, `arrowProps`, and
`viewportProps` remain available for the matching Base UI escape hatches:
