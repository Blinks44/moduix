...
Use `classNames`, `portalProps`, `positionerProps`, `arrowProps`, and `viewportProps` only when
you need slot-level customization.
...
`onOpenChange`, and `handle`. `TooltipContent` renders the portal, positioner, arrow,
viewport, and popup automatically.

`className` styles the visible popup. `classNames` styles the internal service slots that are
hidden from the default composition. Use `container`, `portalProps`, `positionerProps`,
`arrowProps`, `showArrow={false}`, and placement props such as `side` or `sideOffset` when you
need the matching Base UI escape hatches, including `viewportProps` for the viewport wrapper:
