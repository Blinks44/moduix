...
`ToggleGroup` is the Base UI root with group styles. `ToggleGroupItem` is a styled `Toggle` that
gets the group state through Base UI and inherits the group's `variant` and `size` by default.
