...
`Switch` renders the track and internal thumb automatically. Use `SwitchField` and
`SwitchLabel` for the wrapping-label pattern. For sibling labels, use a native `label`
with `id`.
...
`SwitchField`, `SwitchLabel`, and `classNames.thumb`.

## Composition

Use `className` to style the track, `classNames.thumb` to style the internal thumb, and
`thumb` to render custom thumb content such as an icon.
...
| `thumb`      | internal  | Any `ReactNode` rendered inside thumb          |
| `classNames` | undefined | Supports `thumb` key for internal thumb styles |
...
Pass `thumb` to render custom content inside the internal thumb.
