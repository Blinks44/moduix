...
The shape parts are marked `aria-hidden` because they describe loading surfaces, not readable
content.
...
`Skeleton` is composed from visual placeholder blocks and optional layout helpers.
...
shared parent when one screen demonstrates multiple skeleton styles.
...
Use `SkeletonRect` with smaller line skeletons to reserve the shape of card content.
...
Combine `SkeletonCircle`, `SkeletonColumn`, and `grow` for avatar rows, list items, and comments.
...
Use `gap`, `pt`, `pb`, `grow`, and `mobileStack` when the skeleton should preserve loaded spacing.
...
Pass `className` to the root or layout primitives for local styling overrides.
