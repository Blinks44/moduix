...
`Link`) and optional icon slot (`NavigationMenuIcon`). The popup infrastructure (`Portal`,
...
| `NavigationMenuIcon`    | Optional visual indicator for trigger state; accepts custom children for icon replacement. |
...
`NavigationMenu` renders Base UI root state and keeps popup infrastructure out of the
required composition. By default it creates `Portal`, `Positioner`, `Popup`, `Arrow`, and
`Viewport` automatically through `popupContent`.
...
Use `popupContent={false}` and `viewport` when a nested menu should render its content inside
