...
`NavigationMenu` is built from the visible navigation parts (`List`, `Item`, `Trigger`,
`Content`, `Link`) plus the trigger icon slot. Popup infrastructure (`Portal`, `Positioner`,
`Popup`, `Arrow`, `Viewport`) stays internal by default through `popupContent`, so common menus
can use a short composition.
...
In most cases, style the visible parts first. Use `popupContent.className` and
`popupContent.classNames` only when popup infrastructure needs separate styling.
...
`NavigationMenuTrigger`, `NavigationMenuContent`, and `NavigationMenuLink` forward their matching
Base UI props, including `render`, `keepMounted`, `active`, and `closeOnClick`.
...
Use `popupContent` for placement and popup styling. `popupContent.className` styles the visible
popup, while `popupContent.classNames` styles the internal service slots:
...
`popupContent.container` for the portal container, `popupContent.withBackdrop` for a backdrop,
`popupContent.withArrow={false}` to hide the arrow, `popupContent.fullWidth` for viewport-width
popup layout, and `popupContent.slotProps` when a service slot needs non-class props.
...
| `popupContent`                   | `{}`                   | `false` or popup options object |
| `popupContent.sideOffset`        | `10`                   | Number                          |
| `popupContent.withArrow`         | `true`                 | `true`, `false`                 |
| `popupContent.withBackdrop`      | `false`                | `true`, `false`                 |
| `popupContent.fullWidth`         | `false`                | `true`, `false`                 |
| `NavigationMenuTrigger.hideIcon` | `false`                | `true`, `false`                 |
| `NavigationMenuTrigger.icon`     | `ChevronDownSmallIcon` | Any React node                  |
...
<Preview>
            ...
            <div className={styles.container}>
              <NavigationMenu
                ...
                popupContent={{
                  ...
                  sideOffset: 12,
                  withArrow: false,
                  classNames: { positioner: styles.positioner },
                }}
              >
                <NavigationMenuList>
                  <NavigationMenuItem>
  ...
  <Preview.CSS>
    {`
      ...
      .positioner {
        --navigation-menu-full-width-inset: 32px;
      }
