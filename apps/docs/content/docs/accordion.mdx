...
Accordion is composed as a stack of independent items. Each item owns one question-like control and
one answer-like panel; keep the trigger and panel inside the same `AccordionItem` so their state and
accessibility wiring stay paired.
...
| `AccordionHeader`      | Semantic heading wrapper for the trigger. Use it to place the toggle in the document outline.                    |
| `AccordionTrigger`     | The interactive button. It opens or closes the matching panel and receives open/disabled state attributes.       |
| `AccordionTriggerIcon` | Optional visual state marker. It renders the default plus icon, or your custom icon as children.                 |
...
`Accordion` does not use portal-like service layers such as `portal`, `backdrop`, or `viewport`.
In most cases, keep structure and behavior defaults and style only the visible parts:
`AccordionTrigger`, `AccordionTriggerIcon`, and the content wrapper inside `AccordionPanel`.

## Composition

Use `Accordion` for root state and behavior props such as `value`, `defaultValue`,
`onValueChange`, `multiple`, `disabled`, and `orientation`.

All parts are visible slots in the public composition. Style them with `className` on
`Accordion`, `AccordionItem`, `AccordionHeader`, `AccordionTrigger`, `AccordionTriggerIcon`,
and `AccordionPanel`. There are no hidden service slots, so this component does not use
`classNames`.
