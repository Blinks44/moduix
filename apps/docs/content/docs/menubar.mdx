...
Start by styling the visible parts: `Menubar`, `MenubarTrigger`, `MenubarContent`, and
item rows. Reach for `classNames` and `slotProps` only when you need to customize internal
service slots such as `portal`, `backdrop`, `positioner`, or `viewport`.

## Composition

`Menubar` owns the root menubar behavior. Each `MenubarMenu` owns one top-level popup.

`className` styles the visible popup. `classNames` styles internal service slots.
`MenubarContent` forwards Base UI popup props and accepts positioner props such as `side`,
`sideOffset`, `align`, `alignOffset`, `arrowPadding`, `anchor`, `collisionAvoidance`,
`collisionBoundary`, `collisionPadding`, `sticky`, `positionMethod`, and
`disableAnchorTracking`. Arrow is disabled by default; set `withArrow` to enable the
built-in arrow and pass `arrow` to replace its content. Use `container` and `withBackdrop`
for the common cases, and `slotProps` for lower-level portal, backdrop, positioner, arrow,
or viewport props:
...
| `MenubarContent.withArrow`          | `false`                                                     | `true`, `false`          |
| `MenubarContent.withBackdrop`       | `false`                                                     | `true`, `false`          |
| `MenubarSubmenuContent.withArrow`   | `false`                                                     | `true`, `false`          |
