...
└─ MenubarMenu
   ...
   └─ MenubarContent
      ├─ portal (service slot)
      │  └─ positioner (service slot)
      │     ├─ backdrop (optional service slot)
...
| `MenubarContent`                                       | Popup container for menu items and submenus. Accepts placement props and service-slot overrides. |
| `MenubarArrow`                                         | Optional visual arrow that points from popup to trigger.                                         |
| `MenubarItem`/`MenubarCheckboxItem`/`MenubarRadioItem` | Action and stateful command rows.                                                                |
...
`portalProps`/`positionerProps`/`viewportProps`/`backdropProps` only when you need to
customize service layers.
...
`MenubarContent` renders the portal, positioner, viewport, and optional backdrop automatically.
`className` styles the visible popup. `classNames` styles internal service slots hidden from
the default composition. Use `container`, placement props, `portalProps`, `positionerProps`,
`backdropProps`, `viewportProps`, and `withBackdrop` when you need the matching Base UI escape
hatches:
