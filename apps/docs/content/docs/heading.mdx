...
`Heading` renders semantic `h1`-`h6` elements with library heading styles.

Base UI does not ship a standalone heading primitive, so this component stays close to
plain HTML: `as` controls semantics, `size` controls visual scale, and `weight` is a
small styling shortcut for the common emphasis variants.
...
for the common typography presets, and `className` or CSS variables for anything more
specific. `Heading` has no internal slots or structural composition.
...
`Heading` is already the full structure, so the escape hatch is direct styling on the
root element. Pass `className` to add one-off presentation like color or size overrides.
