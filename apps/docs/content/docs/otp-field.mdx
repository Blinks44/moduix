...
| `OTPFieldSeparator` | Optional visual separator between groups.                                                    |
...
When composing manually, keep the first slot unlabeled so it inherits the field label, and give the
remaining slots position-aware `aria-label` values.
