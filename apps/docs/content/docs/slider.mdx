...
`Slider` renders the control, track, and indicator internally. In app code you compose only the
label, optional value, and one thumb per value.
...
The control, track, and indicator stay internal by default. Use `classNames.control`,
`classNames.track`, and `classNames.indicator` when CSS variables are not enough.

## Composition

Use `Slider` for behavior props such as `value`, `defaultValue`, `onValueChange`, `min`, `max`,
`step`, `orientation`, and `thumbAlignment`.
...
Style the root with `className`, visible parts with their own `className`, and internal slots with
`classNames`:
