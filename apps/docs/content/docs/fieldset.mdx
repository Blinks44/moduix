...
`Fieldset` renders a native `fieldset` by default and passes Base UI props through directly. The
main escape hatch is `render`, which lets the same semantic group also act as another Base UI root
such as `RadioGroup` or `CheckboxGroup`.
