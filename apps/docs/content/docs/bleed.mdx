...
Use `as` for semantic/custom roots without changing bleed behavior. `inline="full"` is viewport-based,
...
(`section`, `figure`, etc.). `Bleed` forwards refs and matching element props to the rendered root.
...
Use a block/flex/grid-capable root for predictable layout. Inline roots such as `span` are not a
practical fit for bleed behavior.
