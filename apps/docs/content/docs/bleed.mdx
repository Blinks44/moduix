...
`Bleed` is a moduix layout primitive (not a Base UI wrapper).
Control each axis with `inline` and `block`; use `none` to keep an axis constrained.
Supported amounts: `xs`, `sm`, `md`, `lg`, `xl`, and `full` (inline only).
Use `as` for semantic block-level HTML roots without changing bleed behavior. `inline="full"` is viewport-based,
so it is best suited for full-bleed sections rather than container-matching layout.
...
Use `inline` and `block` to control bleed per axis, and `as` when the root should be semantic
(`section`, `figure`, etc.).
...
| `as`     | `div`   | `div`, `main`, `section`, `article`, `header`, `footer`, `nav`, `aside`, `figure` |

`Bleed` has no internal slots. Customize via `className` and root `--bleed-*` variables.

`Bleed` is a layout primitive, so `as` is intentionally limited to semantic block-level HTML
elements that are a practical fit for bleed behavior.
...
### Full Bleed In Flex Layouts

`inline="full"` keeps working when the parent is a flex column with `align-items: flex-start`, but
it still uses viewport-based sizing. In app shells or nested scroll regions, override the full-bleed
variables locally if your layout needs custom math.
...
<Preview>
  ...
  <Preview.Code>
    {`
      ...
      export function BleedFlexStartDemo() {
        return (
          <div className={styles.flexStartContainer}>
            <Text tone="muted">
              Parent uses flex column with align-items: flex-start.
            </Text>
            <Bleed inline="full" className={styles.panel}>
              <Text>Full inline bleed in flex-start context</Text>
            </Bleed>
            <Text tone="muted">
              Following content stays constrained to parent width.
            </Text>
          </div>
        );
      }
