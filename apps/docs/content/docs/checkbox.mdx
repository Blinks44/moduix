...
Pass `checkedIcon` and `indeterminateIcon` to swap the built-in visuals, `indicator` to replace the
default icon node inside the built-in indicator, or `children` to replace the full default indicator tree.
When both `children` and `indicator` are provided, `children` takes precedence because it fully owns root composition.
The root also forwards its `ref` to the underlying Base UI element for focus management and imperative integrations.
...
| `parent`            | `false`     | `boolean` (for parent checkboxes inside `CheckboxGroup`)           |
| `required`          | `false`     | `boolean`                                                          |
| `nativeButton`      | `false`     | `boolean`                                                          |
| `checked`           | `undefined` | `boolean` (controlled mode, with `onCheckedChange`)                |
| `checkedIcon`       | `undefined` | `ReactNode`                                                        |
| `indeterminateIcon` | `undefined` | `ReactNode`                                                        |
| `indicator`         | `undefined` | `ReactNode`                                                        |
| `children`          | `undefined` | `ReactNode` (manual indicator/root composition)                    |
...
Use `readOnly` when the current state should remain focusable and perceivable but must not change from user input.
...
Pass `children` when you need to replace the full default indicator tree instead of only swapping the icon node.
...
<Preview>
  ...
  <Preview.Code>
    {`
      ...
      export function ManualIndicatorCheckboxDemo() {
        return (
          <CheckboxField>
            <Checkbox defaultChecked>
