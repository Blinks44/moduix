...
composition: `portal`, `backdrop`, `positioner`, and `arrow`. Use `container`, `withBackdrop`,
`withArrow`, and `slotProps` when you need Base UI escape hatches for service slots.
...
| `sideOffset`   | `5`     | `number \| OffsetFunction`                                 |
| `slotProps`    | —       | `{ portal?, backdrop?, positioner?, arrow? }`              |

For overlapping positioning props, the precedence is:
top-level prop on `AutocompleteContent` -> `slotProps.positioner` -> internal default.
For portal container, `container` takes precedence over `slotProps.portal.container`.

Use the `arrow` prop to replace the built-in arrow content. Keep `classNames.arrow` for styling and
`slotProps.arrow` for non-class arrow props.
