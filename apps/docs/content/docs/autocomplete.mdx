...
| `AutocompleteArrow`          | Optional arrow rendered inside `AutocompletePopup`.                                                 |
...
Use `className` for the visible popup and `sideOffset` when the default spacing needs a small
adjustment. When you need a backdrop, custom portal container, arrow styling, or advanced
positioning, compose the infrastructure parts explicitly instead of passing configuration props to
`AutocompleteContent`.
