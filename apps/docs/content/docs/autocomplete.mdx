...
| `AutocompleteFieldLabel`     | Accessible label wired through Base UI field labeling for the input or field trigger.               |
...
| `AutocompleteContent`        | Default popup composition: `AutocompletePortal` -> `AutocompletePositioner` -> `AutocompletePopup`. |
| `AutocompletePortal`         | Optional portal wrapper when you need explicit control over popup placement in the DOM.             |
| `AutocompleteBackdrop`       | Optional overlay rendered inside `AutocompletePortal`.                                              |
| `AutocompletePositioner`     | Optional positioning wrapper for custom placement and collision settings.                           |
| `AutocompletePopup`          | Visible popup surface when composing the infrastructure manually.                                   |
| `AutocompleteArrow`          | Built-in arrow for the popup. Enable it with `showArrow` or replace it in custom composition.       |
...
Use `AutocompleteContent` for the default popup. Reach for `AutocompletePortal`,
`AutocompleteBackdrop`, `AutocompletePositioner`, `AutocompletePopup`, and `AutocompleteArrow`
only when the popup infrastructure itself needs to change.
...
filtering. Use `grid` with `AutocompleteRow` for grid navigation, and `virtualized` when the list
is rendered by an external virtualization layer. `useAutocompleteFilter` provides Base UI string
matching helpers, and `useAutocompleteFilteredItems` returns the filtered result list for custom
rendering.

`AutocompleteContent` is the short path for the common case. It styles the popup surface and keeps
the default `Portal -> Positioner -> Popup` structure.

Use `className` for the visible popup. `AutocompleteContent` also accepts the shared popup sugar:
`side`, `sideOffset`, `align`, `alignOffset`, `arrowPadding`, `collisionAvoidance`,
`collisionBoundary`, `collisionPadding`, and `showArrow`.

When you need a backdrop, custom portal container, custom arrow content, or a different popup
structure, compose the infrastructure parts explicitly instead of passing more props to
`AutocompleteContent`.
