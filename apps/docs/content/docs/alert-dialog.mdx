...
- `withBackdrop={false}` removes only the visual dimming layer; the dialog still stays modal
...
`AlertDialogAction` and `AlertDialogCancel` are convenience close actions with library styling.
Use them when the click may dismiss the dialog immediately.
If the primary action must wait for async work, render your own button and close the dialog from
controlled state or a handle after the request succeeds.
...
Disable backdrop rendering when you want the modal confirmation without page dimming.
This removes only the backdrop layer; the rest of the page still remains non-interactive.
...
<Preview>
  ...
  <Preview.Code>
    {`
      ...
      export function NoBackdropAlertDialogDemo() {
        return (
          <AlertDialog>
