...
Use `AlertDialogContent` for the surface. It already renders the portal, backdrop, viewport, and
popup with the library defaults, so the main customization path is composition inside the content.
...
`AlertDialogContent` accepts `AlertDialog.Popup` props from Base UI. In practice, the important
one for library consumers is `className`, which styles the popup itself.
...
Render `AlertDialogCloseIcon` where you need it. The default close button is already styled, and
you can replace its rendered control through the `render` prop when the icon or button shell
needs to change.
...
Use `className` for popup-level styling and CSS variables for visual changes. That keeps the public
API small while still exposing the usual theming hooks.
