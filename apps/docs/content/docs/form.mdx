  ...
  <Preview.Code>
    {`
      ...
      export function OnFormSubmitDemo() {
        const [errors, setErrors] = useState({} as FormProps["errors"]);
        const [submitting, setSubmitting] = useState(false);
...
<Preview>
  ...
  <Preview.Code>
    {`
      ...
      async function submitUsername(_previousState, formData) {
        await new Promise((resolve) => {
          setTimeout(resolve, 500);
        });
...
<Preview>
  ...
  <Preview.Code>
    {`
      ...
      export function StyledFormDemo() {
        return (
