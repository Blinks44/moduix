...
<Preview cssProperties={formCssProperties}>
  <FormExample />
...
`Form` is a root wrapper around native form semantics and validation orchestration.
...
`Form` renders the native `<form>` and keeps Base UI form behavior available through
`errors`, `actionsRef`, `onFormSubmit`, `validationMode`, `action`, and native form props.
