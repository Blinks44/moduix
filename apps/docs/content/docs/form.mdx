...
`Form` is only the styled root. Use Base UI form props such as `errors`, `actionsRef`,
`onFormSubmit`, `validationMode`, `action`, and native form attributes directly on it.
Everything else stays regular composition with `Field`, `FieldLabel`, `Input`,
