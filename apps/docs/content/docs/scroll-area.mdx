...
`ScrollArea` renders one root plus internal scrolling slots. In most cases you style the root and,
when needed, target internal slots through `classNames`.
...
Use `ScrollArea` for root behavior props such as `scrollbars`, `fade`,
`overflowEdgeThreshold`, and `scrollbarKeepMounted`.
...
| `contentMinWidth`      | `full`     | `full`, `fit-content`                             |
| `scrollbarKeepMounted` | `false`    | `true`, `false`                                   |

Shared `classNames.scrollbar` and `classNames.thumb` are applied before axis-specific classes, so
common styling can stay shared and only per-axis differences need overrides.
...
Set `scrollbars="both"` when content can overflow in both axes. Use
`contentMinWidth="fit-content"` so content keeps its intrinsic width and can actually overflow
horizontally.
...
Set `fade` to fade content near the scroll edges. Tune the size with
`--scroll-area-fade-size`, `--scroll-area-fade-start-size`, and `--scroll-area-fade-end-size`.
Pass `fade="horizontal"` for horizontal overflow or `fade="both"` for both axes.
...
Use `overflowEdgeThreshold` when fade and edge attributes should appear only after a specific
scroll offset.
...
Use `scrollbarKeepMounted` when scrollbar DOM must stay mounted while the viewport switches between
scrollable and non-scrollable states.
