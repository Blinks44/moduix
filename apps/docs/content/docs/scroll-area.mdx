...
`ScrollArea` renders the Base UI root, viewport, content, scrollbars, thumbs, and corner for you.
Use `className` for the root element and `classNames` for the internal slots hidden from the
default composition.
