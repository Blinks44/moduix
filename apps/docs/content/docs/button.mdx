...
When `loading` is enabled without `loadingText`, the original button content stays in the accessibility tree so the button keeps its accessible name.
...
For submit buttons, pass `type="submit"`. For links, style an `<a>` directly. For icon-only buttons, provide an accessible name with `aria-label`.
...
| `focusableWhenDisabled` | `false`           | `true`, `false`                                                                          |
...
Use `render` and `nativeButton={false}` when the button should render another tag but keep button semantics.
...
Use built-in loading props when an action is pending and layout should stay stable. Add `focusableWhenDisabled` when the loading state starts after the button has already received focus.
