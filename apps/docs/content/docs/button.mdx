...
`Button` is one interactive root with optional internal loading slots. In common usage, you
compose content as children and optionally enable loading states.
...
`Button` is built on Base UI Button and keeps its behavior props available, including
`render`, `nativeButton`, `disabled`, and `focusableWhenDisabled`. Use `className` for the
