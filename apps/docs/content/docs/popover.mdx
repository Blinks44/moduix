...
`onOpenChange`, `modal`, and `handle`. `PopoverContent` renders the portal, positioner,
popup, and arrow automatically. Set `withBackdrop` when the popup needs an overlay and
...
`positionerProps`, `viewportProps`, and `arrowProps` when you need the matching Base UI
escape hatches:
