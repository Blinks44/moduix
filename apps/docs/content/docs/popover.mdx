...
viewport, `withArrow={false}` to remove the default arrow, and `arrow` to replace it. Reach for
`slotProps` only when you need Base UI escape-hatch props such as `keepMounted` on the portal or
