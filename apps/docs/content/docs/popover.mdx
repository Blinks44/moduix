...
`withViewport` to wrap content in Base UI's viewport, `arrow={false}` to remove the default
...
Set `arrow={false}` when the popup should look like a floating panel without a pointer.
