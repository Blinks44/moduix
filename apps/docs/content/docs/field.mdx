...
Base UI parts also support `render`, function `className`, function `style`, and state data
...
controls such as select or combobox triggers. `FieldError match` accepts `true` or a
`ValidityState` key such as `valueMissing`, `typeMismatch`, or `customError`. `FieldValidity`
exposes the full validity object through a render function.
...
<Preview>
  ...
  <Preview.Code>
    {`
      ...
      export function CustomCompositionFieldDemo() {
        return (
