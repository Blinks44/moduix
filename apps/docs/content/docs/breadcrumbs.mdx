...
Use `items` to describe the path. Each item can be plain text or a link (`href`).
...
Use `render` to integrate external routing links while keeping component styles.
