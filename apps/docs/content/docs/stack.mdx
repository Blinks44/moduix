...
Use it when the layout intent is just direction, spacing, and common flex alignment.
Keep responsive layout rules and one-off overrides in `className` or `style`.
...
while leaving sizing and responsive layout rules to local CSS.
...
`Stack` does not add responsive behavior or child-level layout abstractions.
Use `className` or `style` when you need media queries, sizing constraints, or custom flex rules.
