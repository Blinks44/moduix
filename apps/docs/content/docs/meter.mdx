...
`Meter` stays close to the Base UI primitive. You compose the visible parts explicitly.
...
| `MeterTrack`     | Visual track behind the fill.                                                                  |
...
Compose `MeterTrack` and `MeterIndicator` directly. Structure and styling stay on the same visible
parts you render.
...
Style each visible part directly with `className`.
