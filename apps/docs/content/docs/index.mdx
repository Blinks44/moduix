...
moduix as a regular npm package and receive updates through package management, or install component
source into your own project with `shadcn` when a project needs direct ownership and local
customization.

The recommended place to start is the [Quick start](/docs/quick-start). It covers both install
paths and documents the current temporary `components.json` workaround for the `shadcn` path.
