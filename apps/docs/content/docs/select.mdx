...
| `SelectContent` | Popup surface plus positioning and service-slot configuration. |
| `SelectItem`    | Selectable option row.                                         |

In most cases, style the visible parts and keep the default popup infrastructure. Reach for
`classNames`, `slotProps`, `withBackdrop`, and `arrow` only when you need custom portal, layering,
...
`className` styles the popup surface. `classNames` targets internal service slots. Put positioning
props directly on `SelectContent`, use `container` for the portal target, and use `slotProps` for
...
| `arrow`            | `—`     | Custom `ReactNode` rendered inside the built-in arrow slot                        |
| `sideOffset`       | `8`     | Any number accepted by Base UI positioner                                         |
| `classNames`       | `—`     | Optional slot classes: `portal`, `backdrop`, `positioner`, `arrow`                |
| `slotProps`        | `—`     | Optional non-class slot props for `portal`, `backdrop`, `positioner`, `arrow`     |
| `container`        | `—`     | Portal mount target. Top-level `container` overrides `slotProps.portal.container` |
| `animation`        | `—`     | `scale`                                                                           |
...
Use `SelectContent` props to configure popup infrastructure. `Portal`, `Positioner`, `Backdrop`,
and `Arrow` stay internal; application code only passes behavior and service-slot classes.
