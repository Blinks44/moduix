...
Use `withArrows` and `withPages` to keep only the controls you need for the layout.
...
`count` is required and clamped to at least `1`. `page` and `onPageChange` are optional and
activate controlled mode when `page` is provided.
