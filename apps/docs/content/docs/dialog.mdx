...
Set `withCloseButton` to render a standard top-right close icon automatically; keep manual
`DialogCloseIcon` composition when you need custom placement or icon replacement. For a viewport-
level close control (outside popup bounds), pass it through `outsideCloseIcon`.
