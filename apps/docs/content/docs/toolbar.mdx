...
`variant`, and `size`. Add `ToolbarGroup` when a subset of controls needs its own accessible
name, and `ToolbarSeparator` when controls should be split into separate clusters.
...
Use `variant` to match the surrounding surface.
...
Use `size` to fit compact or roomy interfaces.
...
Pass `className` to the root and individual parts when styling with CSS Modules, Tailwind CSS, or
CSS-in-JS.
