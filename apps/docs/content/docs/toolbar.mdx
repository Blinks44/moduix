...
Compose `ToolbarButton` with `Toggle` inside a `ToggleGroup` when items need pressed state. Because
this path does not use `ToggleGroupItem`, pass any non-default `Toggle` props such as `variant`
directly to the rendered `Toggle`.
