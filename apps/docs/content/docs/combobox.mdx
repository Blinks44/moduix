...
In most cases, keep default popup infrastructure and style visible parts (`ComboboxInput`,
`ComboboxItem`, `ComboboxItemIndicator`, `ComboboxItemText`, `ComboboxContent`).
Use `ComboboxContent` service-slot props (`classNames`, `slotProps`, `withBackdrop`, `withArrow`)
...
| `sideOffset`   | `5`     | `number`                                                                          |
| `container`    | `body`  | `HTMLElement`, `ShadowRoot`, or a callback returning one of them (`null` allowed) |

`className` styles the visible popup. `classNames` styles the internal service slots that are
hidden from the default composition. Use `container`, `slotProps`, `withBackdrop`, and `withArrow`
for service-slot customization:
...
<ComboboxContent
  ...
  slotProps={{
    ...
    positioner: { side: 'bottom', sideOffset: 8 },
  }}
/>
```
