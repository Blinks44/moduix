...
that composition. `ComboboxFieldLabel` stays associated with the trigger and inline input through
Base UI field labeling, so no extra `htmlFor` wiring is needed here.
