...
`onValueChange`, `open`, `onOpenChange`, `multiple`, and `filter`. `ComboboxContent`
renders the portal, positioner, optional backdrop, popup, and optional arrow automatically.
...
`positionerProps`, `arrowProps`, `withBackdrop`, and `showArrow` when you need the matching
