...
`RadioGroupItemControl` renders the indicator automatically. Use `indicator` or `classNames` only when the default dot needs to change.
...
`Radio` is used inside `RadioGroup`. The recommended composition keeps the row, control, and label explicit while the indicator stays internal.
...
Use `indicator` for custom graphics and `classNames.indicator` or `classNames.indicatorIcon` when the internal indicator needs targeted styling.

## Composition

Put behavior props such as `value`, `defaultValue`, `onValueChange`, `disabled`, `readOnly`, `required`, `name`, and `form` on `RadioGroup`. Use `RadioGroupItemControl` for the common grouped option pattern, `RadioField` for compact enclosing-label rows, and `Radio` with `nativeButton` plus `render={<button />}` for sibling-label composition.

### Defaults

| Prop (Part)                                                    | Default            | Values                       |
| -------------------------------------------------------------- | ------------------ | ---------------------------- |
| `size` (`Radio` / `RadioGroupItemControl`)                     | `md`               | `xs`, `sm`, `md`, `lg`, `xl` |
| `children` (`Radio`)                                           | Internal indicator | Any React node               |
| `indicator` (`Radio` / `RadioGroupItemControl`)                | Internal dot icon  | Any React node               |
| `classNames.indicator` (`Radio` / `RadioGroupItemControl`)     | `undefined`        | `string`                     |
| `classNames.indicatorIcon` (`Radio` / `RadioGroupItemControl`) | `undefined`        | `string`                     |

Visible parts accept `className`. The internal indicator is customized through `indicator` or `classNames`:
...
Control `value` from React state when the selection needs to coordinate with other UI.
...
Set `disabled` on the group to prevent interaction with every option.
...
Pass a custom `indicator` node to `Radio` or `RadioGroupItemControl` to replace the default dot.
...
Pass `className` to visible slots and `classNames` to the internal indicator.
...
Use `RadioField`, `Radio`, and `RadioLabel` directly for a compact enclosing-label composition.
...
Use `Field` and `Fieldset` when the group needs form naming, validation, or shared labeling.
