...
you need full trigger markup control. When using a manual icon inside `children`, set
`withIcon={false}` to avoid rendering the built-in icon twice. Base UI behavior props (`render`, `nativeButton`,
`keepMounted`, `hiddenUntilFound`) pass through to matching parts.

When replacing the trigger element with `render`, `CollapsibleTrigger` keeps its content contract
and only swaps the host element. For fully custom trigger markup, compose your own children and
`CollapsibleTriggerIcon`. If the rendered element is not a native `<button>`, pass
`nativeButton={false}` so Base UI applies the correct ARIA button behavior.
...
Use `render` to swap the trigger element, and set `withIcon={false}` when composing your own
`CollapsibleTriggerIcon` inside custom trigger content.
