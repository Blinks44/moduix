...
loading. Use root-level CSS variables such as `--avatar-size`, `--avatar-font-size`, and
`--avatar-line-height` when the same size system should apply to both image and fallback content.
