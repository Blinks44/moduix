...
`Avatar`, `AvatarImage`, and `AvatarFallback` are all public slots in one composition.
Use `className` on each slot, and use Base UI's `render` prop when the root should compose
