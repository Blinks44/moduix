...
`Toggle` is a single styled root over the Base UI primitive.

- Use `variant` and `size` for the built-in library styles.
- Use `pressed`, `defaultPressed`, and `onPressedChange` for controlled or uncontrolled state.
- Use `render` only when the rendered element or icon needs to depend on primitive state.
- Use `className`, `style`, or CSS variables for visual overrides.
