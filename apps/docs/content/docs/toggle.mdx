...
import {
  ...
  CustomCompositionToggleExample,
...
Use `pressed`, `defaultPressed`, and `onPressedChange` for controlled or uncontrolled state.
Use `disabled` to lock interaction, `render` when the rendered element depends on state, and
`className` or `style` for styling. `Toggle` has a single root slot, so `className` is the main
...
| `defaultPressed` | `false`     | `true`, `false`                                         |
| `disabled`       | `false`     | `true`, `false`                                         |
| `nativeButton`   | `true`      | `true`, `false`                                         |
| `value`          | `undefined` | Any `string`                                            |

## Examples

### Variants

Use `variant` to match the surrounding UI.
...
Use `render` when the icon or rendered element should depend on the pressed state.
...
Pass `className` to layer custom styles on top of the default Toggle styling.
...
<Preview>
  ...
  <Preview.Code>
    {`
      ...
      export function CustomCompositionToggleDemo() {
        return (
