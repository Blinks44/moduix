  ...
  >
    ...
    <CarouselContext v-slot="api">
      <CarouselControl :class="styles.control">
        <CarouselPrevTrigger />
        <CarouselIndicatorGroup>
          <CarouselIndicator v-for="(_, index) in api.pageSnapPoints" :key="index" :index="index" />
        </CarouselIndicatorGroup>
        <CarouselNextTrigger />
      </CarouselControl>

      <div :style="{ 'margin-inline': 'auto' }">
        <Button size="sm" variant="outline" @click="api.scrollToIndex(3)">Go to slide 4</Button>
      </div>
    </CarouselContext>
  </Carousel>
</template>