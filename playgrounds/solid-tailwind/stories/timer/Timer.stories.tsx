...
function TimerItemGroup(props: {
  ...
  return (
    <span class="inline-grid justify-items-center gap-1">
      <Timer.Item class={props.itemClass} type={props.type} />
...
function TimerControls(props: { actionClass?: string }) {
  return (
    <Timer.Control>
      <Timer.ActionTrigger action="start" class={props.actionClass}>
        <PlayIcon /> Start
      </Timer.ActionTrigger>
      <Timer.ActionTrigger action="resume" class={props.actionClass}>
        <PlayIcon /> Resume
      </Timer.ActionTrigger>
      <Timer.ActionTrigger action="pause" class={props.actionClass}>
        <PauseIcon /> Pause
      </Timer.ActionTrigger>
      <Timer.ActionTrigger action="reset" class={props.actionClass}>
        <RotateCcwIcon /> Reset
      </Timer.ActionTrigger>
    </Timer.Control>
  );
...
export const CustomStyling: Story = {
  render: () => (
    <Timer.Root targetMs={15 * 60 * 1000}>
      <Timer.Area class="text-3xl">
        <Timer.Item class="text-primary" type="minutes" />
        <Timer.Separator class="text-chart-2">·</Timer.Separator>
        <Timer.Item class="text-primary" type="seconds" />
      </Timer.Area>
