...
const optionClass = 'flex items-center gap-2 text-sm text-foreground';
    ...
    <Fieldset>
      ...
      {['ssd', 'hdd'].map((value) => (
        <label class={optionClass}>
          <input checked={value === 'ssd'} name="storage-type" type="radio" />
          {value.toUpperCase()}
        </label>
      ))}
...
export const CustomStyles: Story = {
  render: () => (
    <Fieldset class="max-w-64 gap-3 rounded-lg border border-primary/30 p-4">
      <Fieldset.Legend class="ms-2 px-2 text-primary">Styled fieldset</Fieldset.Legend>
      <Field.Root class="gap-2">
        <Field.Label class="text-primary">Project name</Field.Label>
        <Field.Input
          class="border-primary/40 focus-visible:outline-primary"
          placeholder="Maps Platform"
        />
      </Field.Root>
      <Fieldset.HelperText class="text-primary">Visible to project members.</Fieldset.HelperText>
    </Fieldset>
  ),
};