import type { CreateToasterReturn, ToastPlacement } from '@ark-ui/solid/toast';
  ...
  render: () => (
    <>
      ...
      <Toaster toaster={customToaster}>
        {() => (
          <Toast.Root class={customToastClass}>
            <div class={customContentClass}>
              <InfoIcon class={customIconClass} />
              <Toast.Title />
              <Toast.Description class="text-primary-foreground/72" />
            </div>
            <Toast.CloseTrigger class="!text-primary-foreground [@media(hover:hover)]:hover:!bg-primary-foreground/14 [@media(hover:hover)]:hover:!text-primary-foreground">
...
function ToastRenderer(props: { toaster: ToastToaster }) {
  return <Toaster toaster={props.toaster} />;
}

function PlacementStory() {
  const [placement, setPlacement] = createSignal<ToastPlacement>('bottom-end');
  ...
  return (
    <>
      ...
      {placements.map((item) => (
        <ToastRenderer toaster={placementToasters[item]} />
      ))}
    </>
  );
}
