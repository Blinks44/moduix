...
test('sizes the default submenu chevron to its icon wrapper', () => {
  const Harness = defineComponent({
  ...
  expect(icon).toHaveClass(styles.iconSvg);
});
...
test('applies consumer classes alongside CSS Module defaults', () => {
  const Harness = defineComponent({
    components: menuComponents,
    template:
