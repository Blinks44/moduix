...
test('preserves value text anatomy and consumer classes', () => {
  const Harness = defineComponent({
    components: clipboardComponents,
    template: `
      <Clipboard class="consumer-root" default-value="moduix/clipboard">
        <ClipboardControl class="consumer-control">
          <ClipboardValueText class="consumer-value" />
          <ClipboardTrigger class="consumer-trigger"><ClipboardIndicator /></ClipboardTrigger>
        </ClipboardControl>
      </Clipboard>
    `,
  });
  ...
  const valueText = container.querySelector('[data-slot="clipboard-value-text"]');
  ...
  expect(valueText?.className.endsWith('consumer-value')).toBe(true);
