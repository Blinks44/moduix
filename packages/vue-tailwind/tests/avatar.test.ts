...
test('preserves Ark anatomy, attrs, consumer classes, size, and Vue refs', () => {
  ...
  expect(root).toHaveClass('relative', 'inline-flex', 'rounded-full', 'size-control-lg');
  ...
  expect(fallback).toHaveClass('inline-flex', 'size-full', 'rounded-[inherit]');
  ...
  expect(image).toHaveClass('block', 'size-full', 'object-cover');
...
test('merges consumer Tailwind classes after component defaults', () => {
  const Harness = defineComponent({
    components: avatarComponents,
    template:
      '<Avatar class="rounded-none bg-primary"><AvatarFallback>AT</AvatarFallback></Avatar>',
  });

  render(Harness);
  const root = screen.getByText('AT').closest('[data-slot="avatar-root"]') as HTMLElement;

  expect(root).toHaveClass('rounded-none', 'bg-primary');
  expect(root).not.toHaveClass('rounded-full', 'bg-muted');
});
