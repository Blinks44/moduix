...
Solid uses Ark's native render-function `asChild` API. As with other Ark Solid primitives, refs are
forwarded on native roots but are not forwarded through an `asChild` render callback.
