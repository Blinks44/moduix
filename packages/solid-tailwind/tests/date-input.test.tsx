...
test('lets consumer utilities replace component defaults', () => {
  const { container } = render(() => (
    <DateInput class="w-80 max-w-sm gap-4">
      <DateInput.Label>Release date</DateInput.Label>
      <DateInput.Control class="w-80 rounded-lg bg-muted px-0 text-primary">
        <DateInput.SegmentGroup class="gap-2">
          <DateInput.SegmentContext>
            {(segment) => <DateInput.Segment segment={segment} class="min-w-0 px-0" />}
          </DateInput.SegmentContext>
