...
test('forwards refs to ordinary parts', () => {
  ...
  expect(addonRef.current).toBe(screen.getByText('@'));
  expect(inputRef.current).toBe(screen.getByRole('textbox', { name: 'Workspace' }));
  expect(textRef.current).toBe(screen.getByText('.com'));
  expect(buttonRef.current).toBe(screen.getByRole('button', { name: 'Copy' }));
});
