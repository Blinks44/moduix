...
test('lets consumer Tailwind classes override conflicting defaults', () => {
  render(
    <ImageCropper className="gap-0" aria-label="Landscape crop">
      <ImageCropperViewport>
        <ImageCropperImage src="/landscape.jpg" />
        <ImageCropperCropArea />
      </ImageCropperViewport>
...
test('keeps component-owned visual utilities on their owning parts', () => {
  const { container } = render(
    <ImageCropper aria-label="Landscape crop">
      <ImageCropperViewport>
        <ImageCropperImage src="/landscape.jpg" />
  ...
  expect(viewport).toHaveClass('border', 'border-border', 'rounded-lg', 'bg-background');
  expect(selection).toHaveClass('border-2', 'border-primary', 'rounded-md');
  expect(grids[0]).toHaveClass(
    'pointer-events-none',
    'data-[axis=horizontal]:border-y',
    'data-[axis=horizontal]:border-white/62',
  );
  expect(grids[1]).toHaveClass(
    'pointer-events-none',
    'data-[axis=vertical]:border-x',
    'data-[axis=vertical]:border-white/62',
  );
  expect(handle).toHaveClass('size-3', 'border', 'bg-white/96');
});
