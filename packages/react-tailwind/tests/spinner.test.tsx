...
test('renders the default status with stable styling hooks', () => {
  ...
  expect(spinner.querySelector('[data-slot="spinner-ring"]')).toHaveClass(
    'size-full',
    'rounded-full',
    'border-2',
    'border-solid',
...
test('lets a consumer class override the selected size utility', () => {
  render(<Spinner decorative size="lg" className="size-10" data-testid="spinner" />);

  expect(screen.getByTestId('spinner')).toHaveClass('size-10');
  expect(screen.getByTestId('spinner')).not.toHaveClass('size-7');
});
