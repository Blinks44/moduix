...
const ChartTooltipContent = forwardRef<HTMLElement, HTMLArkProps<'div'>>(
  function ChartTooltipContent({ className, ...props }, ref) {
    return (
      <ark.div
        ...
        className={clsx(styles.tooltipContent, normalizeClassName(className))}
        {...props}
      />
    );
  },
  ...
  if (typeof content === 'string') {
    return (
      <div data-slot="chart-tooltip-body" className={styles.defaultTooltipContent}>
        <span data-slot="chart-tooltip-text">{content}</span>
      </div>
    );
  }
  ...
  return (
    <ChartPrimitive
      ...
      renderTooltipBody={(context) => {
        const defaultBody = renderDefaultTooltipBody(context);

        return renderTooltipBody ? (
          renderTooltipBody({ ...context, defaultBody })
        ) : (
          <ChartTooltipContent>{defaultBody}</ChartTooltipContent>
        );
      }}
